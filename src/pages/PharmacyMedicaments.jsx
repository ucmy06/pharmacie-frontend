...
export default function PharmacyMedicaments() {
    ...
    const fetchMedicaments = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          // Récupérer le nom de la pharmacie depuis une autre requête si nécessaire
          const pharmacyResponse = await axiosInstance.get(`/api/pharmacies/${pharmacyId}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          if (pharmacyResponse.data.success) {
            setPharmacyName(pharmacyResponse.data.data.pharmacie.pharmacieInfo.nomPharmacie || 'Pharmacie');
          }
        } else {
